...
async function sendViaProvider(
  ...
  if (smsProvider === 'twilio') {
    return sendViaTwilio(to, body, statusCallbackUrl)
  }
  return sendViaMessageBird(to, body, statusCallbackUrl)
}
...
export async function sendSMS({
  ...
  // Dry run mode: log to DB but don't call provider
  if (isDryRun) {
    console.log(`[SMS DRY RUN] [${smsProvider}] Would send SMS:`)
  ...
  try {
    const statusCallbackUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/sms/webhook`
    const message = await sendViaProvider(normalizedTo, body, statusCallbackUrl)
    ...
    console.error(`[SMS] [${smsProvider}] Send error:`, error.message)
