...
import { CheckCircle, ChevronDown, ChevronUp, Car, User, Clock, Phone, Wrench } from 'lucide-react'
import Image from 'next/image'
import { toast } from 'sonner'

interface RepairEstimateService {
  id: number
  title: string
...
interface RepairEstimateData {
  ...
  services: RepairEstimateService[]
}
...
interface RepairEstimateClientProps {
  estimate: RepairEstimateData
  token: string
  shopProfile?: ShopProfile | null
}

export function RepairEstimateClient({ estimate, token, shopProfile }: RepairEstimateClientProps) {
      ...
      <div className="max-w-lg mx-auto px-4 pt-5">
        <div className="bg-card rounded-xl border border-border p-5 mb-5">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            that need attention. Review each item below and choose what works for you.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-2 space-y-3">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold text-foreground">
            Repair Services ({estimate.services.length})
          </h2>
          <button
        ...
        {estimate.services.map((service) => {
          ...
          const needsExpand = explanation.length > 150 || !!service.techNotes
          ...
          return (
            ...
            >
              ...
              {/* Expanded content: tech notes + photo */}
              {isExpanded && (
                <div className="px-4 pb-4 space-y-3">
                  {service.techNotes && (
                    <div className="bg-muted/50 rounded-lg p-3">
                      <p className="text-xs font-medium text-muted-foreground mb-1">Technician Notes</p>
                      <p className="text-sm text-foreground leading-relaxed">{service.techNotes}</p>
                    </div>
                  )}
              ...
              {needsExpand && (
                ...
                >
                  {isExpanded ? (
                    ...
                    <>
                      {service.techNotes || service.photoPath ? 'See details' : 'Read more'}
