/**
 * GET /api/repair-estimate/[token]
 *
 * Public endpoint - customer views repair estimate details.
 * No auth required (token-based access).
 * Simplified version of /api/estimates/[token] — no vehicle diagram/hotspots.
 * Includes tech_notes and photo_path from linked recommendations.
 */
...
export async function GET(
  ...
  try {
    ...
    // Fetch estimate with customer and vehicle
    const estimateResult = await query(`
      SELECT
        ...
        v.year, v.make, v.model, v.vin, v.mileage
      FROM estimates e
    ...
    // Fetch services with tech_notes and photo_path from linked recommendations
    const servicesResult = await query(`
      SELECT es.id, es.service_title, es.customer_explanation, es.estimated_cost, es.status,
             vr.tech_notes, vr.photo_path
